<?php
    ...
    class Skeleton{
        ...
        /**
         * Dumps the database to an SQL file.
         * @param string $target Target directory, relative to the **app** folder. Must have writing permissions.
         * @param bool $createTables (Optional) Include CREATE TABLE statements.
         * @param bool $createDatabase (Optional) Include CREATE DATABASE statements.
         * @param bool $dropTables (Optional) Drop existing tables before creating them.
         * @param bool $dropDatabase (Optional) Drop the existing database before creating it.
         * @param bool $includeData (Optional) Include whole database data in the dump.
         * @param bool $deleteData (Optional) Delete table data before inserting new.
         * @param string $type (Optional) Insert type. Valid types are: `INSERT`, `INSERT IGNORE` or `REPLACE`.
         */
        public function dumpSQL(string $target = 'storage', bool $createTables = true, bool $createDatabase = false, bool $dropTables = false, bool $dropDatabase = false, bool $includeData = true, bool $deleteData = false, string $type = 'INSERT'){
            // Validate directory
            $target = Util::location(trim($target, '/\\'));
            if(!is_writable($target)) throw new FileException('Directory "' . $target . '" is not writable, please check your chmod settings');
            $dbName = Config::get("database.{$this->_connection}.db");
            $target = $target . DIRECTORY_SEPARATOR . Util::kebabCase($dbName) . '_' . date('Y_m_d_His') . '.sql';

            // Prepare result
            $result = '-- --------------------------------------------------------' . "\n";
            $result .= '-- MySQL server:      ' . $this->getConnection()->host_info . "\n";
            $result .= '-- Server version:    ' . $this->getConnection()->get_server_info() . "\n";
            $result .= '-- Generated in:      ' . date('m/d/Y H:i:s P e') . "\n";
            $result .= '-- Dump generated by: Glowie Framework v' . Util::getVersion() . "\n";
            $result .= '-- --------------------------------------------------------' . "\n\n";
            $result .= '/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;' . "\n";
            $result .= '/*!40101 SET NAMES ' . $this->getConnection()->character_set_name() . ' */;' . "\n";
            ...
            if($dropDatabase) $result .= "-- Deleting database {$dbName}\nDROP DATABASE IF EXISTS `{$dbName}`;\n\n";

            // CREATE DATABASE statement
            if($createDatabase) $result .= "-- Creating database {$dbName}\nCREATE DATABASE IF NOT EXISTS `{$dbName}`;\n\n";

            // Get tables
            $tables = $this->asArray()->query('SHOW TABLES', true);
            $tables = array_column($tables ?? [], 'Tables_in_' . $dbName);

            if(!empty($tables)){
                foreach($tables as $table){
                    // DROP TABLE statement
                    if($dropTables) $result.= "-- Deleting table {$table}\nDROP TABLE IF EXISTS `{$table}`;\n\n";
                    ...
                    if($createTables){
                        ...
                        if(!empty($createQuery[0]['Create Table'])){
                            ...
                            $createQuery = Util::replaceFirst($createQuery[0]['Create Table'], 'CREATE TABLE', 'CREATE TABLE IF NOT EXISTS');
                            $result .= $createQuery . ";\n\n";
                        }
                    }
                    ...
                    if(!empty($data)){
                        ...
                        if ($deleteData) $result .= "-- Deleting data from {$table}\nTRUNCATE TABLE `{$table}`;\n\n";
                        ...
                        $result .= $type . ' INTO `' . $table . '` (`' . implode('`, `', $columns) . "`) VALUES";
