<?php
    ...
    class Skeleton{
         ...
         */
        public function dumpSQL(string $target = 'storage', bool $createTables = true, bool $createDatabase = false, bool $dropTables = false, bool $dropDatabase = false, string $type = 'INSERT'){
            // Validate directory
            ...
            if($dropDatabase) $result .= "DROP DATABASE IF EXISTS `{$dbName}`;\n\n";

            // CREATE DATABASE statement
            if($createDatabase) $result .= "CREATE DATABASE IF NOT EXISTS `{$dbName}`;\n\n";

            if(!empty($tables)){
                foreach($tables as $table){
                    // DROP TABLE statement
                    if($dropTables) $result.= "DROP TABLE IF EXISTS `{$table}`;\n\n";
